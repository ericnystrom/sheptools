...
- `shep-add-ruletype`: adds the "ruletype" column, based on Linda Berger's headnote analysis (data not included here) (Awk)
- `shep-add-scdb`: adds SCDB case ID numbers, extracting them from the well-formed download file names rather than SCDB-provided data (Awk)
- `shep-add-vote`: add column of SCDB data indicating Supreme Court vote (note, must extract said data from SCDB) (Awk)
- `shep-basicgraph`: uses `ploticus` to produce a basic graph of citations by year, up to 4 cases (Bash + Awk + ploticus)
- `shep-concat-by-ref`: pulls together cite-disaggregated data into case-level data (Awk)
- `shep-count-ruletype-cites`: For each case/headnote combo, outputs a line showing case, headnote, ruletype, and count of headnote-citations. (Awk)
- `shep-de-duplicate`: de-duplicate on the basis of identical records with different "shepnum" records (gawk-specific)
- `shep-extract-headnotes`: extract LexisNexis Headnotes from HTML-exported case text from Lexis-Nexis Academic (Perl)
- `shep-extract-summary`: extract case summary from HTML-exported case text from Lexis-Nexis Academic (Perl)
- `shep-group`: add a group column based on jurisdiction to the output
  of a Shepards breakdown generated by shep2csv. Different jurisdiction maps can be specified; grouping maps for controlling
  circuit (`MAP-control-circ.tsv`), for court level
  (`MAP-court-levels.tsv`), and for various Federal/State breakdowns
  (`MAP-fed-state.tsv`, `MAP-levels-fedappeals.tsv`), are provided. (Awk)
- `shep-group-howused`: add a "howused" or treatment mapping
  column. Different treatment maps can be specified,
  `MAP-howused-basic.tsv` which maps to Positive, Negative, Neutral,
  including "Cited" as Neutral, is included. (Awk)
...
- `shep-hn-pct`: from results of `shep-hn-pivot`, normalize citations by year (Awk)
- `shep-hn-pivot`: a pivot-tables style yearly aggregator for headnote-separated output, to aggregate by year. (Awk)
- `shep-howused-pivot`: pivot-tables style aggregator for usage treatments case and year (Awk)
- `shep-opinion-age`: calculate and add an approximate "opinion age"
  for each citation in `shep2csv` output containing SCDB info (Awk)
...
**Dependencies:** `shep2csv`, `shep-extract-headnotes`, and `shep-extract-summary` depend on
...
`libtext-trim-perl` packages respectively. `shep-basicgraph` requires
the program `ploticus` (contained in the Debian package of the same
name). The other tools need Awk -- they were developed using `gawk` but should work with other forms of
Awk, except for `shep-de-duplicate` which requires `gawk` specifically.

## Typical usage

NOTE: The interface that provided suitable HTML output reports has
been deprecated; this information is provided for historical use only.
